...
static void ByteReverse (unsigned char* buf, unsigned longs)
{
    uint32_t t = 0;
    do {
        t = static_cast<uint32_t>(((unsigned)buf[3] << 8 | buf[2]) << 16 |
...
{
    const unsigned char *in_buf = reinterpret_cast<const unsigned char*>(data.data ());
...
static inline std::string&& MD5String (const StringPiece& str)
{
    ...
    return std::move (MD5DigestToString (digest));
}
...
{
    return std::move (detail::MD5DigestToString (digest_));
}
...
{
    if (data && length) {
        StringPiece str (reinterpret_cast<const char*>(data), length);
        out = std::move (detail::MD5String (str));
    }
}
